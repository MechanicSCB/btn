    ...
    >
        <div @click.stop class="relative w-full max-w-[750px] max-h-screen min-h-[400px] text-white bg-[#374050] py-5 px-3 cursor-default overflow-y-auto">
            ...
            <div class="absolute top-4 right-1 flex items-center">
                <PencilIcon v-if="$page.props.auth.user" @click="showedProductId=null;editedProductId=product.id" class="p-1 w-5 h-5 bg-gray-900 cursor-pointer fill-gray-400 hover:fill-yellow-500"/>
                <TrashIcon v-if="$page.props.auth.user" @click="deleteProduct" class="ml-0.5 p-1 w-5 h-5 bg-gray-900 cursor-pointer fill-gray-400 hover:fill-red-500"/>
